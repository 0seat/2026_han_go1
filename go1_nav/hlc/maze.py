...
TILE = 2.0                  #: 랜드 한 변 (m)
#: 경사 랜드의 기울기. **20.0 에서 내렸다 (2026-08-30).**
#:
#: 20 도는 Go1 의 등판 한계 25 도에서 5 도를 남긴 값이었다. 등판만 보면 맞는
#: 값이다. 문제는 등판이 아니라 **횡단**이었다.
#:
#: 근거 (`slope_test.py`, 경사 한 칸, 400 스텝, 진행축 전진 m):
#:
...
#: 두 가지가 한꺼번에 걸려 있었다.
#:
#:     좌우 비대칭   오르막이 **왼쪽**이면 20 도에서 vx 0.6 부터 넘어진다.
#:                   넘어지기 전 요각이 +66 도까지 돈다. 오른쪽이면 멀쩡하다.
#:                   미로 실측도 같다 -- 같은 경사 타일 81 개를 방향만 바꿔
#:                   밟은 짝에서 도달 0.115 대 0.876 이다
#:     등반 자체     20 도에서 명령 0.73 m/s 에 실제 0.145 m/s 다. 미로에서
#:                   정면 오르막 도달이 0.377 이던 이유가 이것이다
#:
#: 12 도에서 좌우가 대칭으로 돌아온다. 등반도 같이 살아난다.
#:
#: **12 가 아니라 10 을 쓴다.** 한 칸 복도로는 12 와 10 이 같아 보였는데,
#: 미로에서 다시 재니 12 는 아직 모자랐다 (씨앗 0 · 64x64 · 같은 850 차선):
#:
#:     차선        20 도   12 도   10 도
#:     경사 없음  0.875   0.870   0.873      <- 각도와 무관. 부작용이 없다
#:     경사 있음  0.467   0.797   0.885      <- 12 도는 아직 7%p 모자랐다
#:     전체       0.768   0.849   0.878
#:
#: 경사 없는 차선이 셋 다 같다는 것이 요점이다. 각도를 내린 것이 다른 지형에
#: 아무 영향이 없고, 12 도에 남아 있던 차이가 **정말로 경사 탓**이었다는 뜻이다.
#: 10 도에서 경사 차선이 경사 없는 차선을 넘어선다.
#:
#: LLC 를 다시 학습시킬 여유가 없어 지형을 실제 능력에 맞췄다. 난이도를 낮춘
#: 것이 아니라, 20 도 횡단이 "도전"이 아니라 "불가능"이었다.
#: `footswing` 이나 횡경사 보상이 열리면 되돌린다.
#:
#: **주의 -- 높이 배열은 정규화 [0, 1] 이고 실제 높이는 `값 * SPAN` 이다.**
#: 배열 차이를 그대로 미터로 읽으면 각도가 3.38 배 작게 나온다. 실측으로
#: 확인할 때 한 번 속았다.
SLOPE_DEG = 10.0
...
HIGH = (CELLS_PER_TILE - 1) * CELL * math.tan(math.radians(SLOPE_DEG))  #: 0.346 m
...
#: 그런데 `SPAN = ELEVATION + DEPTH` 이고 `SPAN` 은 **관측 서명에 들어간다**
#: (`obs._signature_fields` 의 "높이범위"). 유도된 채로 두면 `SLOPE_DEG` 를
#: 건드리는 순간 서명이 바뀌어 지금까지 학습한 파라미터를 전부 못 읽는다.
#:
#: 고정값은 `SLOPE_DEG = 20.0` 시절의 값 그대로다. 높이 격자의 값은 [0, 1] 이고
#: 실제 높이가 `값 * SPAN` 이므로, 이 수를 붙들어 두면 **격자 값의 뜻이 안 바뀐다.**
#: 지형만 낮아지고 관측 단위는 그대로다.
#:
#: 남는 여유는 손해가 아니다. hfield 값이 float 라 눈금이 굵어지지 않는다.
#: 아래 assert 가 필요한 높이를 실제로 담는지 확인한다 (12 도에서 1.633).
ELEVATION = 2.276763318323513
